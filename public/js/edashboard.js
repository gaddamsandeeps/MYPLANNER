...
app.controller('genericCtrl', function($scope, $rootScope, Services, $http) {
	...
	$scope.displayChart = function(pid, pname){
		Services.getURL('/getDetailedProjectReportLogs?id='+pid).then(function(d) {
			var usersData = [],billable='Billable',nonBillable = 'Non Billable';
			...
			for (var i in d) {			
			  ...
			  generateCharts(pid, pname+' Resource Spent Hours ( Billable vs Non-Billable )', usersData);
			  if($('#'+pid).hasClass('hidechart')){
				$('#'+pid).removeClass('hidechart');
			  } else {
				$('#'+pid).addClass('hidechart');
			  }
        }, function(e) {
            console.log(e);
        });
