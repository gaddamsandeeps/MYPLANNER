...
----------------------------------------------------------------------------------------------------------------------*/
                    ...
                    $.each(this.split(','), function(j, buttonName) {
                        ...
                        if (buttonName == 'title') {
                            ...
                            if (buttonClick) {
                                ...
                                classes = ['fc-' + buttonName + '-button',
                                    tm + '-button',
                                    tm + '-state-default'
                                ];
                                button = $('<button class="' + classes.join(' ') + '">' + innerHtml + '</button>').click(function() {
    ...
    function EventManager(options) { // assumed to be a calendar
        ...
        var cache = [];
        $.each(
            (options.events ? [options.events] : []).concat(options.eventSources || []), function(i, sourceInput) {
        ...
        function mutateEvent(event, newStart, newEnd) {
            ...
            if (newAllDay != oldAllDay) {
                ...
                durationDelta = dayishDiff(
                    // new duration
                    newEnd || t.getDefaultEventEnd(newAllDay, newStart || oldStart), newStart || oldStart).subtract(dayishDiff(
...
----------------------------------------------------------------------------------------------------------------------*/
    ...
    ------------------------------------------------------------------------------------------------------------------*/
        ...
        renderHelper: function(event, sourceSeg) {
            ...
            this.rowEls.each(function(row, rowNode) {
                ...
                skeletonEl.css('top', skeletonTop).find('table').append(tbodyEls[row]);
    ...
    ------------------------------------------------------------------------------------------------------------------*/
        ...
        renderHighlight: function(start, end) {
            ...
            for (i = 0; i < segs.length; i++) {
                seg = segs[i];
                el = $(this.highlightSkeletonHtml(seg.leftCol, seg.rightCol + 1) // make end exclusive
                );
    ...
    $.extend(DayGrid.prototype, {
        ...
        renderEventRows: function(events) {
            ...
            for (row = 0; row < segRows.length; row++) {
                ...
                tbodyNodes.push(this.renderSegSkeleton(segRows[row])[0]);
            }
            return {
        ...
        renderSegHtml: function(seg) {
            ...
            '</span>';
            return '<a class="' + classes.join(' ') + ' '+logStatus+'"' + (event.url ? ' href="' + htmlEscape(event.url) + '"' : '') + (skinCss ? ' style="' + skinCss + '"' : '') + '>' + '<div class="fc-content">' + (isRTL ? titleHtml + ' ' + timeHtml : // put a natural space in between
                timeHtml + ' ' + titleHtml //
            ) + '</div>' + (isResizable ? '<div class="fc-resizer"/>' : '') + '</a>';
        },
    ...
    $.extend(TimeGrid.prototype, {
        ...
        slatRowHtml: function() {
            ...
            while (slotTime < this.maxTime) {
                ...
                axisHtml = '<td class="fc-axis fc-time ' + view.widgetHeaderClass + '" ' + view.axisStyleAttr() + '>' + ((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
...
----------------------------------------------------------------------------------------------------------------------*/
    $.extend(TimeGrid.prototype, {
        ...
        renderSegHtml: function(seg) {
            ...
            return '<a class="' + classes.join(' ') + '"' + (skinCss ? ' style="' + skinCss + '"' : '') + '>' + '<div class="fc-content">' + (timeText ? '<div class="fc-time" data-full="' + htmlEscape(fullTimeText) + '">' + '<span>' + htmlEscape(timeText) + '</span>' + '</div>' : '') + (event.title ? '<div class="fc-title">' + htmlEscape(event.title) + '</div>' : '') + '</div>' + '<div class="fc-bg"/>' + (isResizable ? '<div class="fc-resizer"/>' : '') + '</a>';
        },
...
----------------------------------------------------------------------------------------------------------------------*/
    ...
    View.prototype = {
        ...
        init: function() {
            ...
            $(document).on('mousedown', this.documentMousedownProxy = $.proxy(this, 'documentMousedown')).on('dragstart', this.documentDragStartProxy = $.proxy(this, 'documentDragStart')); // jqui drag
        },
...
----------------------------------------------------------------------------------------------------------------------*/
    ...
    $.extend(BasicView.prototype, {
        ...
        renderHtml: function() {
            return '' + '<table>' + '<thead>' + '<tr>' + '<td class="' + this.widgetHeaderClass + '">' + this.dayGrid.headHtml() + // render the day-of-week headers
                '</td>' + '</tr>' + '</thead>' + '<tbody>' + '<tr>' + '<td class="' + this.widgetContentClass + '">' + '<div class="fc-day-grid-container">' + '<div class="fc-day-grid"/>' + '</div>' + '</td>' + '</tr>' + '</tbody>' + '</table>';
        },
...
----------------------------------------------------------------------------------------------------------------------*/
    ...
    $.extend(BasicWeekView.prototype, {
        ...
        render: function(date) {
            ...
            this.title = this.calendar.formatRange(this.start, this.end.clone().subtract(1), // make inclusive by subtracting 1 ms
                this.opt('titleFormat'), ' \u2014 ' // emphasized dash
            );
...
----------------------------------------------------------------------------------------------------------------------*/
    ...
    ------------------------------------------------------------------------------------------------------------------*/
        ...
        renderHtml: function() {
            return '' + '<table>' + '<thead>' + '<tr>' + '<td class="' + this.widgetHeaderClass + '">' + this.timeGrid.headHtml() + // render the day-of-week headers
                '</td>' + '</tr>' + '</thead>' + '<tbody>' + '<tr>' + '<td class="' + this.widgetHeaderClass + '">' + (this.dayGrid ? '<div class="fc-day-grid"/>' + '<hr class="' + this.widgetHeaderClass + '"/>' : '') + '<div class="fc-time-grid-container">' + '<div class="fc-time-grid"/>' + '</div>' + '</td>' + '</tr>' + '</tbody>' + '</table>';
        },
        // Generates the HTML that will go before the day-of week header cells.
        ...
        // Queried by the DayGrid subcomponent when generating rows. Ordering depends on isRTL.
        dayIntroHtml: function() {
            return '' + '<td class="' + this.widgetHeaderClass + ' fc-axis" ' + this.axisStyleAttr() + '>' + '<span>' + // needed for matchCellWidths
